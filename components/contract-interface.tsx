...
export default function ContractInterface() {
  ...
  const { submitTransaction, connected, account, wallet } = useWallet()
  ...
  useEffect(() => {
    const facilityFromUrl = searchParams.get("facility_address")
    if (facilityFromUrl) {
      setFacilityAddress(facilityFromUrl)
      setInputFacilityAddress(facilityFromUrl)
    }

    const moduleFromUrl = searchParams.get("module_address")
    if (moduleFromUrl) {
  ...
  const handleTransactionSubmit = useCallback(
    async (functionName: string, args: unknown[]) => {
      ...
      try {
        ...
        const result = await aptosClient.submitTransaction(walletAccount, payload)
  ...
  const handleTransactionSimulate = useCallback(
    async (functionName: string, args: unknown[]): Promise<SimulationResult> => {
      if (!walletAccount) {
      ...
      return await aptosClient.simulateTransaction(walletAccount, payload)
    },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 bg-gray-50 rounded-lg">
        ...
        <WalletSelector onConnect={setWalletAccount} connectedAccount={walletAccount} />
      </div>

      {connected && account && (
        <div className="p-4 border rounded-lg bg-blue-50">
          <h3 className="text-lg font-medium mb-2">Connected Account</h3>
          <p className="text-sm mb-1"><strong>Address:</strong> {account.address.toString()}</p>
          <p className="text-sm mb-1"><strong>Network:</strong> {wallet?.name || "Unknown"}</p>
          <p className="text-sm"><strong>Wallet:</strong> {wallet?.name || "Unknown"}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 border rounded-lg">
          ...
          <p className="text-sm text-gray-500 mb-4">Set the contract module address</p>
          ...
          {moduleAddress && <p className="mt-2 text-sm text-green-600">Current module address: {moduleAddress}</p>}
        </div>
